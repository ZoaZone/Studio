...
export default function MediaStudio() {
  ...
  const handleExecutePipeline = async () => {
    setLoading(true);
    try {
      // Adding the missing metadata to prevent the 400 Error
      const payload = { 
      ...
      alert("Pipeline Error: Ensure backend service is reachable."); 
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ padding: "40px", maxWidth: "1200px", margin: "0 auto", fontFamily: "sans-serif" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "350px 1fr", gap: "40px" }}>
        ...
        <div style={{ background: "#f9f9f9", padding: "20px", borderRadius: "12px", border: "1px solid #eee" }}>
          ...
          <select onChange={(e) => setFormData({...formData, format: e.target.value})} style={{ width: "100%", marginBottom: "15px" }}>
        ...
        <div style={{ background: "#000", borderRadius: "12px", minHeight: "300px", display: "flex", alignItems: "center", justifyContent: "center" }}>
          {project ? (
            ...
            <p style={{ color: "#fff" }}>{loading ? "AI is generating..." : "Enter vision to start."}</p>
          )}
        </div>
      </div>
    </div>
