...
/**
 * submitDubVideo — server-side proxy that hands an AI video-dubbing job
 * (ElevenLabs Dubbing API, optionally followed by Replicate lip-sync and
 * ffmpeg caption burn-in) off to the standalone render worker
 * (server-render/, deployed separately, e.g. on Railway — the same worker
 * submitRender, submitMusic, and submitVideo use, just its /dub-video
 * endpoint) and returns the job id it assigns.
 *
 * Mirrors submitDubAudio/entry.ts exactly: same Deno.serve handler shape,
 * same CORS/OPTIONS handling, same createClientFromRequest +
 * base44.auth.me() auth guard, same { error } shape on failure.
 *
 * The request body IS the dub spec as-is ({ sourceUrl, targetLang,
 * sourceLang, numSpeakers, dropBackgroundAudio, disableVoiceCloning,
 * watermark, highestResolution, startTime, endTime, lipSync, burnCaptions,
 * captionOverrides } — see server-render/dub.js's dubVideo) — this function
 * doesn't interpret it, just forwards it to the worker with the shared
 * secret it needs to accept the job.
 */
...
// for that one run. Video dubbing needs both keys: ElevenLabs for the dub
// itself, and (when lipSync is requested) Replicate for the lip-sync step.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      workerRes = await fetch(`${workerUrl.replace(/\/+$/, '')}/dub-video`, {
        method: 'POST',
        headers: {
