...
/**
 * getVideoStatus — polls the standalone render worker for a per-scene
 * video-generation job's current status, on behalf of the frontend (which
 * never talks to the worker or its shared secret directly).
 *
 * Mirrors getMusicStatus/entry.ts exactly: same Deno.serve handler shape,
 * same CORS/OPTIONS handling, same createClientFromRequest +
 * base44.auth.me() auth guard, same { error } shape on failure. The job
 * record shape is identical to a render/music job's — GET /jobs/:id on the
 * worker is shared by all job kinds.
 */
