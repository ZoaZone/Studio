...
/**
 * submitLane1Video — server-side proxy that hands a Lane 1 short-video
 * assembly job (Quick Create, Campaign Studio, Demo Video Maker — see
 * server-render/lane1.js for the schema) off to the standalone render
 * worker (server-render/, deployed separately, e.g. on Railway — the same
 * worker submitRender uses, just its /lane1-video endpoint) and returns
 * the job id it assigns.
 *
 ...
 *
 * The request body IS the project JSON as-is ({ scenes, ratio, resolution,
 * audioMode, voiceoverUrl, musicUrl }) — this function doesn't interpret
 * it, just forwards it to the worker with the shared secret it needs to
 * accept the job.
 */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      workerRes = await fetch(`${workerUrl.replace(/\/+$/, '')}/lane1-video`, {
        method: 'POST',
        headers: {
