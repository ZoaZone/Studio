...
/**
 * submitDubAudio — server-side proxy that hands an AI audio-dubbing job
 * (ElevenLabs Dubbing API) off to the standalone render worker
 * (server-render/, deployed separately, e.g. on Railway — the same worker
 * submitRender, submitMusic, and submitVideo use, just its /dub-audio
 * endpoint) and returns the job id it assigns.
 *
 * Mirrors submitVideo/entry.ts exactly: same Deno.serve handler shape, same
 ...
 * sourceLang, numSpeakers, dropBackgroundAudio, disableVoiceCloning } — see
 * server-render/dub.js's dubAudio) — this function doesn't interpret it,
 * just forwards it to the worker with the shared secret it needs to accept
 * the job.
 */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      workerRes = await fetch(`${workerUrl.replace(/\/+$/, '')}/dub-audio`, {
        method: 'POST',
        headers: {
      ...
      // submitMusic's, and submitVideo's, since it's the same underlying
      // worker.
